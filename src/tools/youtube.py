...
def get_music_metadata(url: str) -> dict:
    ...
    args = [
        '--cookies-from-browser', 'brave::Personal',
        '--quiet',
        '--no-warnings',
        '--ignore-errors',
        '--print-json',
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if info is None:
                return {"error": "Could not fetch metadata for this URL. It might be private, restricted, or deleted."}
            
            formats = [
                {"format_id": f.get("format_id"), "ext": f.get("ext"), "quality": f.get("quality"), "note": f.get("format_note")}
...
@MCP_SERVER.tool()
def download_music(url: str, output_dir: str = "~/Downloads", quality: str = "320") -> str:
    """
    ...
    args = [
        ...
        '--embed-thumbnail',
        '--add-metadata',
        '--output', os.path.join(expanded_dir, '%(title)s.%(ext)s'),
        '--quiet',
        '--no-warnings',
        url
    ]
